...
public class SelectMenuPage extends BasePage {
    ...
    public SelectMenuPage verifyColor() {
        ...
        Assert.assertTrue(shouldHaveText(oldSelectMenu, firstSelectedOption, 0));
        return this;
    }

    @FindBy(id = "react-select-4-input")
    WebElement intputSelect;
    @FindBy(css = "html")
    WebElement space;
    ...
    public SelectMenuPage multiSelect(String[] colors) {
        for (int i = 0; i < colors.length; i++) {
            if (colors[i] != null) {
                intputSelect.sendKeys(colors[i]);
                intputSelect.sendKeys(Keys.ENTER);
            }
            click(space);
        }
        return this;
    ...
    public SelectMenuPage standardMultiSelectTest(String[] cars) {
